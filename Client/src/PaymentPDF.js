...
function PaymentPDF({ address, totalAmount,totalAmt, list, details }) {
  const generatePDF = () => {
    ...
    doc.text(`Amount: ${totalAmount}`, 10, paymentInfoStartY + 10);
    doc.text(`GST Calculation: SGST + CGST + IGST: ${gst}`, 10, paymentInfoStartY + 20);
    doc.text(`Mode Of Delivery: ${details.deliveryType}`, 10, paymentInfoStartY + 30);
    doc.text(`Payment Type: ${details.paymentType}`, 10, paymentInfoStartY + 40);
    doc.text(`Address: ${address.addressLine1} ${address.addressLine2} ${address.addressLine3}`, 10, paymentInfoStartY + 50);
    doc.text(`Pincode: ${address.pincode}`, 10, paymentInfoStartY + 60);
    doc.setFont('helvetica', 'bold');
    doc.setFontSize(25); 
    doc.text(`Total Amount of Payment: ${totalAmount}`, 9, paymentInfoStartY + 80);
